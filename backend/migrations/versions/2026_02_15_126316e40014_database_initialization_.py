...
Revision ID: 126316e40014
Revises:
Create Date: 2026-02-15 23:17:58.970014
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("project_id", sa.UUID(), nullable=False, comment="FK to project"),
        sa.Column(
            "name",
            sa.Enum("GITLAB", "JIRA", "SONARQUBE", "LOGFIRE", "INFISICAL", name="integration_type"),
            nullable=False,
            comment="Integration name (e.g., GITLAB, INFISICAL)",
        ),
        sa.Column(
            "external_id", sa.String(length=255), nullable=False, comment="External ID in the integration system"
        ),
        sa.Column(
            "created_at",
            sa.TIMESTAMP(timezone=True),
        ...
        sa.Column("is_active", sa.Boolean(), server_default="true", nullable=False, comment="Soft-delete flag"),
        sa.ForeignKeyConstraint(
            ["project_id"], ["project.id"], name=op.f("fk_integration_project_id_project"), ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_integration")),
        comment="External integration references for projects",
    )
